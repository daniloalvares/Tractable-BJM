functions{
// ------------------------------------------------------
//                GAMMA SURVIVAL SUBMODEL                
// ------------------------------------------------------
    // Gamma hazard function
    vector loghazGamma(vector t, real eta, real nu){
         vector[num_elements(t)] out;
         for(i in 1:num_elements(t)){
            out[i] = gamma_lpdf(t[i] | nu, 1/eta) - gamma_lccdf(t[i] | nu, 1/eta);
         }
         return out;
    }                                                                                     

    // Gamma cumulative hazard function
    vector cumhazGamma(vector t, real eta, real nu){
         vector[num_elements(t)] out;  
         for(i in 1:num_elements(t)){
            out[i] = -gamma_lccdf(t[i] | nu, 1/eta);
         }
         return out;
    }
// ------------------------------------------------------ 
...
// ------------------------------------------------------
{
   ...
   // Log-hazard function
   lhaz = loghazGamma( t2, eta, nu ) + lmodelh[indobs];
   // Log-survival function
   lsurv = -rows_dot_product(cumhazGamma( t1, eta, nu ), exp(lmodelh - lmodelt));
